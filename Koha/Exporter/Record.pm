    ...
    if ($conditions) {
        foreach my $condition (@{$conditions}) {
            ...
            if (!@fields) {
                ...
                if ($operator eq '?') {
                    return undef unless any { $subfield ? $_->subfield($subfield) : $_->data() } @fields;
                } elsif ($operator eq '!?') {
                    return undef if any { $subfield ? $_->subfield($subfield) : $_->data() } @fields;
                } else {
                    my $op;
            ...
            return undef if $no_target && $operator ne '!=';
        }
    }
